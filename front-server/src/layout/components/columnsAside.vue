			...
			<ul @mouseleave="onColumnsAsideMenuMouseleave()">
				...
				>
					<div v-if="!v.meta.isLink || (v.meta.isLink && v.meta.isIframe)" :class="themeConfig.columnsAsideLayout">
						<svg-icon :icon-name="v.meta.icon" />
...
import { reactive, ref, onMounted, nextTick, getCurrentInstance, watch, onUnmounted } from 'vue'
...
const { routesList, isColumnsMenuHover, isColumnsNavHover } = storeToRefs(useRoutesList())
// 定义接口来定义对象的类型
...
const onColumnsAsideMenuMouseenter = (v: RouteRecordRaw, k: number) => {
	...
	useRoutesList().setColumnsMenuHover(false)
	useRoutesList().setColumnsNavHover(true)
}
// 鼠标移走时，显示原来的子级菜单
const onColumnsAsideMenuMouseleave = async () => {
	await useRoutesList().setColumnsNavHover(false)
