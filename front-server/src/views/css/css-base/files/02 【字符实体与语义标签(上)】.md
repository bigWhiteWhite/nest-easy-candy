...
> 个人理解 name 里面的值是指定的，然后 content 里的内容会在根据 name 中的值发挥不同的作用
...
- - description：表示网站的描述信息 ![image-20220701132327433](https://i0.hdslb.com/bfs/album/78204d2bad9627e60c00f167068dca0cb956979a.png)
...
|  | 标签 | 作用 | 描述 |
| --- | --- | --- | --- |
| 块元素 Block Element | `<h1>` `<h2>` `<h3>` `<h4>` `<h5>` `<h6>` | 标题 | 一共有六级标题 从`h1` ~ `h6`重要性递减，`h1`最重要，`h6`最不重要 h1 在网页中的重要性仅次于`title`标签 一般情况下一个页面中只会有一个`h1` 一般情况下标题标签只会使用到`h1` ～ `h3`，`h4` ～ `h6`很少用 |
...
- 将图片使用 base64 编码，这样可以将图片转换为字符，通过字符的形式来引入图片 图片格式的选择
...
通过`iframe`的方式引入视频。以某艺为例，提供了视频链接的 HTML 代码和通用代码
...
<audio controls="controls">
	...
	您的浏览器暂不支持 <audio> 标签。 </audio></audio
>
```
...
<video controls="controls" width="300">
	...
	您的浏览器暂不支持 <video> 标签播放视频 </video></video
>
```
