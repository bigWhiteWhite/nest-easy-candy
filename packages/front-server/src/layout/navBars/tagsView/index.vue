		...
		<el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
			<ul ref="tagsUlRef" class="layout-navbars-tagsview-ul" :class="setTagsStyle">
				...
				>
					...
					<svg-icon v-if="!isActive(v) && getThemeConfig.isTagsviewIcon" :icon-name="v.meta.icon" class-name="pr5"></svg-icon>
					<span>{{ setTagsViewNameI18n(v) }}</span>
					<template v-if="isActive(v)">
						<svg-icon
							icon-name="ele-RefreshRight"
							class-name="ml5 layout-navbars-tagsview-ul-li-refresh"
							@click.stop="refreshCurrentTagsView($route.fullPath)"
						></svg-icon>
						<svg-icon
							v-if="!v.meta.isAffix"
							icon-name="ele-Close"
							class-name="layout-navbars-tagsview-ul-li-icon layout-icon-active"
					...
					<svg-icon
						v-if="!v.meta.isAffix"
						icon-name="ele-Close"
						class-name="layout-navbars-tagsview-ul-li-icon layout-icon-three"
						@click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)"
					></svg-icon>
				</li>
			</ul>
		</el-scrollbar>
		<Contextmenu ref="contextmenuRef" :dropdown="state.dropdown" @current-contextmenu-click="onCurrentContextmenuClick" />
	</div>
</template>
...
const addBrowserSetSession = (tagsViewList: Array<object>) => {
	localCache.set('tagsViewList', tagsViewList, 'session')
}
...
const initTagsView = async () => {
	if (localCache.get('tagsViewList', 'session') && getThemeConfig.value.isCacheTagsView) {
		state.tagsViewList = await localCache.get('tagsViewList', 'session')
	} else {
...
const closeOtherTagsView = (path: string) => {
	if (localCache.get('tagsViewList', 'session')) {
		state.tagsViewList = []
		localCache.get('tagsViewList', 'session').map((v: any) => {
			if (v.meta.isAffix && !v.meta.isHide) {
				v.url = setTagsViewHighlight(v)
...
proxy.$mittBus.on('switchSystem', async () => {
	closeAllTagsView()
})
// 6、开启当前页面全屏
const openCurrenFullscreen = async (path: string) => {
	...
	useTagsViewRoutes().setCurrenFullscreen(true)
}
...
const getCurrentRouteItem = (path: string, cParams: any) => {
	const itemRoute = localCache.get('tagsViewList', 'session') ? localCache.get('tagsViewList', 'session') : state.tagsViewList
...
const tagsViewmoveToCurrentTag = () => {
	nextTick(() => {
		...
		// 当前滚动条的值
		const scrollRefs = proxy.$refs.scrollbarRef.$refs.wrap$ || proxy.$refs.scrollbarRef.$refs['wrapRef']
		// 当前滚动条滚动宽度
