...
一开始的想法是使用用户 id 作为图片名称，这样每一次上传图片，都会把原来的图片覆盖掉。但是这样会有两个问题
...
2. 将图片名称分割为数组，取出 id 与传入的 id 进行判断，符合条件则调用 `fs` 内置模块中的 `fs.unlink()` 方法删除文件
