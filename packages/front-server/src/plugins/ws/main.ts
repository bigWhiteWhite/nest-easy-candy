...
import { websocketOpts } from './main.type'
import { App } from 'vue'
...
export default {
	install(app: App, connection: string, opts: websocketOpts = { format: '' }): void {
		...
		if (opts.connectManually) {
			app.config.globalProperties.$connect = (connectionUrl: string = connection, connectionOpts: websocketOpts = opts) => {
				connectionOpts = { ...opts, ...connectionOpts }
		...
		app.config.globalProperties.$disconnect = () => {
			...
			if (app.config.globalProperties.$socket) {
				...
				app.config.globalProperties.$socket.disconnect() // socket.io
				app.config.globalProperties.$socket.close() // websocket
