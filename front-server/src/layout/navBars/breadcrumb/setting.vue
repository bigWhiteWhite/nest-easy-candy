				...
				<div class="layout-breadcrumb-seting-bar-flex mt15">
					...
					<div class="layout-breadcrumb-seting-bar-flex-value">
						<el-select v-model="getThemeConfig.tagsStyle" placeholder="请选择" size="default" style="width: 90px" @change="setLocalThemeConfig">
							<el-option label="style-one" value="tags-style-one"></el-option>
							<el-option label="style-two" value="tags-style-four"></el-option>
							<el-option label="style-three" value="tags-style-five"></el-option>
						</el-select>
					</div>
				</div>
				<div class="layout-breadcrumb-seting-bar-flex mt15">
...
<script lang="ts" setup name="layoutBreadcrumbSetting">
import { nextTick, onUnmounted, onMounted, getCurrentInstance, computed, reactive } from 'vue'
...
const onSetLayout = (layout: string) => {
	localCache.set('oldLayout', layout, 'local')
...
const setLocalThemeConfig = () => {
	...
	localCache.set('themeConfig', getThemeConfig.value, 'local')
}
// 存储布局配置全局主题样式（html根标签）
const setLocalThemeConfigStyle = () => {
	localCache.set('themeConfigStyle', document.documentElement.style.cssText, 'local')
}
// 一键复制配置
const onCopyConfigClick = () => {
	const copyThemeConfig = localCache.get('themeConfig', 'local')
...
onMounted(() => {
	nextTick(() => {
		...
		if (!localCache.get('frequency', 'local')) initLayoutChangeFun()
		localCache.set('frequency', 1, 'local')
		...
		setTimeout(() => {
			...
			onWartermarkChange()
			// 语言国际化
			if (localCache.get('themeConfig', 'local')) proxy.$i18n.locale = localCache.get('themeConfig', 'local').globalI18n
			// 初始化菜单样式等
			initSetStyle()
		}, 100)
	})
